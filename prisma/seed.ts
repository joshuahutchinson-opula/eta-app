...
import { PHOTO_SPOTS } from './photo-spots'
import { EXPERIENCE_ROUTES, writeExperienceRoute } from './experience-routes'
...
async function main() {
  ...
  await prisma.photoSpot.createMany({ data: withMedia(PHOTO_SPOTS) })

  // Experiences are multi-stop routes of the vendors and photo spots above.
  for (const exp of await prisma.experience.findMany({ where: { name: { in: Object.keys(EXPERIENCE_ROUTES) } } })) {
    await writeExperienceRoute(prisma, exp.id, exp.name)
  }
